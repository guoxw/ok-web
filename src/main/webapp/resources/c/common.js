...
// Debug
C.print = function(){ console && console.log(arguments) };
C.time = function(name, time, func){
...
C.app = (function(){
	 ...
	 * @example 
	 *  	C.app.url('main/default/index');
	 *  	// ==> localhost/?r=main/default/index
	 *  	C.app.url('main/default/index', { op: "add" });
	 ...
	 * @example
	 * 	C.app.getAssetUrl("main", "/js/index.js");
	 * @return
	 */
...
C.settings = {
	...
	dataTable: {
		...
		// --- Language
		language: $.extend({}, C.l("DATATABLE"), {
			processing: '<img src="' + C.app.getStaticUrl("/image/loading.gif") + '">' //"加载中..."
		}),
...
// @deprecated 
C.Event = function($ctx, type, flag){
	var that = this;
	type = type || "click";
	flag = flag || type;
	...
	$ctx.on(type, "[data-" + flag + "]", function(){
		var evtName = $.attr(this, "data-" + flag),
			params = C.app.getEvtParams(this);
...
(function(window, $){
	...
	(function(){
		...
		U.setCookie = function(name, value, seconds, path, domain, secure) {
			...
			path = path || C.app.g('cookiePath') || "";
			domain = domain || C.app.g('cookieDomain') || "";
			expires.setTime(expires.getTime() + seconds * 1000);
			document.cookie = escape((C.app.g('cookiePre') || "") + name) + '=' + escape(value) + 
	...
	(function(){
		...
		C.core.inherits(Ui.PopMenu, Ui.Menu);
		Ui.PopMenu.defaults = {
			position: {
				at: "left bottom",
				my: "left top"
		...
		Ui.showCreditPrompt = function() {
			...
			if(!C.app.g('uid') || remind.length < 2 || remind[6] !== C.app.g('uid')) {
				_reset();
				return;
			}

			names = C.app.g('creditRemind').split(',');
				...
				callbacks: {
					filter: function(query, data, search_key){
						...
						for (_i = 0, _len = data.length; _i < _len; _i++) {
							...
							else if(C.matchSpell(query, item[search_key])){
								_results.push(item);
							}
						}
						return _results
	 ...
	 * @return
	 */
	C.checkFormChange = function(context, tip, selectors){
...
$.fn.waiting = function(content, mode, lock){
	...
	content = content || "";
	mode = mode || "normal";
	var tpl = '<div><img src="<%= C.app.getStaticUrl(path) %>" width="<%= size %>" height="<%= size %>" />' +
...
// @deprecated 使用 C.cmList 或 dataTable 替换
C.List = (function(){
...
// @deprecated 使用 C.cmList 或 dataTable 替换
C.OrderList = function($elem, tpl, options){
	options = {} || options;
...
C.core.inherits(C.OrderList, C.List);

$.extend(C.OrderList.prototype, {
	addItem: function(data, prepend) {
		data = $.extend({
			index: this.index
		}, data);
...
// 通用列表类，提供列表的增删除改功能
C.CmList = function(container, opts){
	this.$container = $(container);
	this.opts = $.extend({}, this.constructor.defaults, opts);
}
C.CmList.prototype = {
	constructor: C.CmList,
